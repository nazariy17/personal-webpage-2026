...
function About() {
  return <section className="about section" id="about"><Reveal className="about-card"><span className="small-label">ENGINEERING / ENTREPRENEURSHIP</span>{profile.portraitImage ? <img className="about-portrait" src={profile.portraitImage} alt={`Portrait of ${profile.name}`} /> : <div className="card-monogram" aria-hidden="true">N.</div>}<div className="card-caption"><strong>{profile.name}</strong><span>Engineer & founder</span></div></Reveal><Reveal className="about-copy"><p className="eyebrow">01 / INTRODUCTION</p><h2>Engineering, product,<br/><span>and perspective.</span></h2>{profile.bio.map(text => <p key={text}>{text}</p>)}<div className="disciplines">{[['Full-stack', 'Web & services'], ['Mobile', 'iOS development'], ['Founder', 'Business & delivery']].map(([title, description]) => <div key={title}><strong>{title}</strong><span>{description}</span></div>)}</div></Reveal></section>;
}
